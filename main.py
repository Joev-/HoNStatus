	...
	while True:
		try:
			...
			if login_status == "Up" and chat_status == "Up" and hon_monitor.client.is_logged_in():
				...
				timer = 0
				while hon_monitor.client.is_logged_in():
					timer += 1
					if timer in [30, 60, 90, 120, 150, 180, 210, 240, 270, 300]:
						log.debug("Tick")
					if timer >= 300:
						hon_monitor.disconnect_logout()
					else:
						if not hon_monitor.client.is_logged_in():
							log.info("Server connection broken, bad test.")
							hon_monitor.disconnect_logout()
						else:
							time.sleep(1)
