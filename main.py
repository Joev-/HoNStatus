...
#	client.configure(chatport=settings['chatport'], chatver=settings['chatver'], invis=settings['invis'],
#					masterserver=settings['masterserver'], basicserver=settings['basicserver'], honver=settings['honver'])

def configure():
	""" Settings from a basic config file """
	log.info("Loading config from file")
	acc_config['username'] = config.username
	acc_config['password'] = config.password
	client.configure(chatver=config.chatver, invis=config.invis)
...
def login_test():
	...
	try:
		response = client.login(acc_config['username'], acc_config['password'])
		status = "Up"
...
def main():
	...
	while True:
		try:
			...
			login_status, login_reason = login_test()
			log.info("Login server:	" + login_status + " - " + login_reason)
			chat_status, chat_reason = chat_test()
			log.info("Chat server:	" + chat_status + " - " + chat_reason)
			
			if login_status == "Up" and chat_status == "Up" and client.is_logged_in():

				# Set a timer to logout the account and disconnect from the chat server in 5 minutes (300 seconds).
				#timer = threading.Timer(300, disconnect_logout)
				#timer.daemon = True
				#timer.start()
...
if __name__ == "__main__":
	...
	log.add_logger('honstatus_mon.log', 'INFO', False)
	
	# Daemonise the program.
	daemon = HSDaemon('tmp/honmon.pid')
	if len(sys.argv) == 2:
		if sys.argv[1] == 'start':
			daemon.start()
